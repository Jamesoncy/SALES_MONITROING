...
class Pm {
	...
	////////////////////////////////////////////////////////////////////////////////////////////
		...
		public function srsTaggedSuppliersDrop($label=null,$name=null,$value=null,$placeholder=null,$tags=array(),$return=false){
			...
			$opts = array(); $vendor = array();
			$CI->session->set_userdata('vendor',null);
			 $manager = $CI->po_model->get_manager($user_id);
       
             if(!$manager){
               $check = $CI->po_model->select_user_vendor($user_id);
                foreach ($check as $val) {
                array_push($vendor, $val->vendor);
              }
              $CI->session->set_userdata('vendor',$vendor);
             }
		...
		public function excludeBranchesDrop($label=null,$name=null,$value=null,$placeholder=null,$tags=array(),$return=false){
			$CI =& get_instance();
			$CI->load->model('srs/po_model');
			$user_roles = $CI->po_model->exclude_main_get_branch_list();
			$opts = array();
			foreach ($user_roles as $val) {
				$opts[$val->name]= $val->code;
			}
		...
		public function blankDrop($label=null,$name=null,$value=null,$placeholder=null,$tags=array(),$return=false){
			
			
			
			$here = $this->selectGroup($label,null,$name,$value,$placeholder,$tags,true);
			if($return) return $here; else $this->code .= $here;
		}
